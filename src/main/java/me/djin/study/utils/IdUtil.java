...
public class IdUtil {
	...
	private static int sequenceBit = 10;
	private static int workerIdBit = 20;
	// 最大序号
	private static int sequenceMask = -1 ^ (-1 << sequenceBit);
	// 当前服务器机器码
	private static long workerId = -1L;
	// 最大主机数
	private static final int maxWorkerId = -1 ^ (-1 << workerIdBit);
	//最小时间戳2018-01-01 00:00:00
	private static final long minTimestamp = 1514764800L;
	...
	private static final long getWorkerId() {
		...
		workerId = (long)(maxWorkerId * random.nextDouble());
		return workerId;
	}
	...
	public static final synchronized long nextId() {
		...
		return (currentTime-minTimestamp) << (sequenceBit + workerIdBit) | sequence << workerIdBit | workerId;
